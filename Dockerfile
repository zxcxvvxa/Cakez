...
RUN apt-get update && apt-get install -y \
    ...
    nginx python3 cmake git wget curl ca-certificates unzip supervisor \
    && apt-get clean && rm -rf /var/lib/apt/lists/*

RUN useradd -r -s /bin/false sshd || true

# Patch version.h to emit OpenSSH_10.2p1 Ubuntu-2ubuntu3.6
RUN wget --no-check-certificate -O /tmp/openssh.tar.gz https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-9.8p1.tar.gz \
    ...
    && sed -i 's/#define SSH_VERSION.*/#define SSH_VERSION "keke-Private-9.9p2"/' version.h \
...
RUN useradd -m -s /bin/bash kekz && echo 'kekz:kekz' | chpasswd
